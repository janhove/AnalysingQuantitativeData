...
\citet{Berthele2011b} asked Swiss teachers in training to rate the 
...
In order to obtain the difference between the estimated conditional
...
@
  \begin{enumerate}
    \item Compute the mean outcome value for each of the four cells (AX, AY, BX, BY).
...
groups and the continuous predictor.  
(We haven't discuss what `statistically significant' means, though.)
\citet{Gelman2006} and \citet{Nieuwenhuis2011} explain why this is a terrible idea.
...
<<eval = FALSE >>=
int.mod <- lm(CorrectSpoken ~ n.Male * EnglishOverall, data = d)
@

  The notation above is equivalent to the following:
<<eval = FALSE>>=
d$Interaction <- d$n.Male * d$EnglishOverall
int.mod <- lm(CorrectSpoken ~ n.Male + EnglishOverall + Interaction, data = d)
@
  In the output of the first model, the \texttt{Interaction} term 
  will be shown as \texttt{n.Male:EnglishOverall}.
