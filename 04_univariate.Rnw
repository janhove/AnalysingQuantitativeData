...
\mypar[Densities]{Exercise}
  ...
  Compute the densities for each bins without drawing the histogram.
...
Third, the Central Limit Theorem does not apply to the sample median.\footnote{Some \clt-like statement for the sample median does, in fact, exist. But it applies only to distributions with a density function, and it requires knowledge of this density function.}
...
This quantity is called the \term{uncorrected (sample) variance}, sometimes written as $s_n^2$,
...
about 8.17:
<<>>=
mean((d$Norwegian - mean(d$Norwegian))^2)
mean(d$Norwegian^2) - mean(d$Norwegian)^2
@
