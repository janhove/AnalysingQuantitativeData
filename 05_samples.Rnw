...
Finally, the Gauss--Markov theorem tells us that, among all unbiased
...
\mypar[Gauss--Markov]{Theorem}\label{th:gauss}
  ...
  Then each unbiased estimator $g(X_1, \dots, X_n)$ of $\mu$ satisfies
  \[
    \Var(\overline{X}) \leq \Var(g(X_1, \dots, X_n)). \parendeq
  \]

It is, however, sometimes possible to come up with unbiased estimators
...
\mypar{Example}
  ...
  of $p$.
  Among all unbiased estimators of $p$, it has the lowest variance.
\parend
...
To convince yourself of this, consider a distribution with $\mu = 0$.}
