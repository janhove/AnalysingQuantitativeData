...
in experimental research. If you have ever taken another introductory
...
\mypar[One- and two-sided $p$-values]{Remark}\label{remark:onetwosided}
  ...
  but the latter way may yield somewhat smaller $p$-values in other cases.
\parend
...
(I have no idea where this misunderstanding comes from, 
but I once saw it in a review report.)
\end{itemize}
...
researchers typically conclude that
the data are too unlikely under the null hypothesis, 
...
In practice, complications can arise.
Exercise \ref{ex:multipletesting} deals with one such complication;
more subtle but equally important ones are discussed in Chapter \ref{ch:QRP}.
...
\mypar[Difference between more than two group means]{Exercise}\label{ex:randtest_multiple}
  ...
  this will give close to the same result,
...
\mypar[Experiment with blocking]{Exercise}
  ...
  \begin{enumerate}
    \item There are $\binom{2n}{n}$ possible ways of dividing $2n$ participants
          into two equally sized groups without any restrictions.  
